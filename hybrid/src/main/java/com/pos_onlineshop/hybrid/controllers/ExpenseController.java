...
public class ExpenseController {
    ...
    public ResponseEntity<?> create(@Valid @RequestBody CreateExpenseRequest request,
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED).body(expenseService.createExpense(request, creatorId));
        } catch (IllegalArgumentException e) {
    ...
    public ResponseEntity<?> approve(@PathVariable Long id, @AuthenticationPrincipal UserDetails principal) {
        try {
            ...
            return runTransition(() -> expenseService.approveAndPay(id, approverId));
        } catch (IllegalStateException e) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Map.of("error", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> reject(@PathVariable Long id, @Valid @RequestBody RejectManualJournalRequest request,
        ...
        try {
            Long approverId = actorResolver.requireActingUserId(principal);
            return runTransition(() -> expenseService.reject(id, approverId, request.getReason()));
        } catch (IllegalStateException e) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Map.of("error", e.getMessage()));
        }
    }
